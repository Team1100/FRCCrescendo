...
public class ShooterSlowOut extends Command {
  ...
  public ShooterSlowOut() {
    super(Shooter.getInstance(), "Basic", "ShooterForwardMove");
  ...
  public void execute() {
    if (m_enablePID.get() == 1) {
      m_shooter.setSpeeds(m_RPM.get(), m_RPM.get(), false);
    }
    else {
      m_shooter.spinOut(m_shooterSpeed.get(), m_shooterSpeed.get());
    }
  }
